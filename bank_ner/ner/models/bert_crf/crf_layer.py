...
def log_sum(matrix, dim):
    ...
    where clip = max
    '''
    clip_value = torch.max(matrix) # 取matrix中的最大值
    clip_value = int(clip_value.data.tolist()) # 转成int格式
...
def argmax(matrix, dim=0):
    '''取matrix的最大值的index， 例如: (0.5, 0.4, 0.3) 获得最大值0.5的index为0'''
...
class CRF(nn.Module):
    ...
    def all_possible_path_score(self, features):
        '''
        计算所有可能的路径分数的log和：使用前向算法
        step1: 将forward列expand成 3 * 3
        step2: 将下个单词的emission行expand成 3 * 3
        step3: 将step1 和 step2 和 对应位置的转移矩阵相加
        step4: 更新forward，合并行
        step5: 取forward指数的对数计算total
        '''
        time_steps = features.size(0)
        # 初始化：初始化START_TAG的发射分数为0
