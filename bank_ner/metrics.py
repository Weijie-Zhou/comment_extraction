...
class EntityScore:
    ...
    def single_entity_score(tags_predict, tags_actual):
        ...
        tags标签体系：BMESO
        '''
        assert isinstance(tags_predict[0], list) and isinstance(tags_actual[0], list) # 确保内层是list
        # 将2维列表展平成1维列表，便于计算
        ...
        regex = re.compile('BI*')
        entity_predict = ['{}_{}'.format(m.start(), m.start() + len(m.group())) for m in regex.finditer(tags_predict)]
        entity_autual = ['{}_{}'.format(m.start(), m.start() + len(m.group())) for m in regex.finditer(tags_actual)]
        # 统计3个数
        predict_num = len(entity_predict)
        actual_num = len(entity_autual)
        correct_num = len(set(entity_predict) & set(entity_autual))
        # 统计准确率、召回率、f1
    ...
    @staticmethod
    def multi_entities_score(tags_predict, tags_actual, entity_types=('BANK', 'PRODUCT', 'COMMENTS_N', 'COMMENTS_ADJ')):
        '''多实体同时计算score'''
        metric = {}
        for entity_type in entity_types:
            # 将其他实体的标签置为O
            cur_tags_predict, cur_tags_actual = copy.deepcopy(tags_predict), copy.deepcopy(tags_actual) # 深拷贝
        ...
        metric['marco_avg']['f1'] = 2 * metric['marco_avg']['precision'] * metric['marco_avg']['recall'] / (metric['marco_avg']['precision'] + metric['marco_avg']['recall']) if (metric['marco_avg']['precision'] + metric['marco_avg']['recall']) else 0.
        return metric